...
for ibatch in range(len(reference["energy"])):
    ...
    for iforce in range(len(reference["forces"][ibatch])):
        for xyz in range(3):
            forces_qc.append(reference["forces"][ibatch][iforce][xyz])
            forces_nn.append(predictions["forces"][ibatch][iforce][xyz])
...
os.system('echo "MAE Energy %f" > %s/MAE.txt'%(MAE,path))
os.system('echo "MAE Forces %f" >> %s/MAE.txt'%(MAEf,path))
